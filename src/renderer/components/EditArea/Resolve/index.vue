					...
					<div class="record-detail-content">
						...
						<div class="image-preview">
							...
							<div class="image-action bt1">
								...
								<v-btn
									:ripple="false"
									@click="replaceImageWithScreenshot(action.resolve)"
									class="cut-button br1"
									flat
								>
									<i class="ms-Icon ms-Icon--Cut"></i>
								...
								<v-btn
									:ripple="false"
									@click="replaceImageWithFile(action.resolve)"
									class="cut-button"
									flat
								>
									<i class="ms-Icon ms-Icon--Upload"></i>
...
export default {
	...
	methods: {
		...
		insertAssert() {
			...
			this.actionList.splice(this.focused, 0, assert);
			this.initAssert();
		},
		showPropertyDialog(actionId, propertyName) {
			...
			this.property.content = editingAction.resolve.property[propertyName].value;
			this.property.showDialog = true;
		},
		comfirmEdit() {
			this.property.editingAction.resolve.property[this.property.name].value = this.property.content;
			this.property.showDialog = false;
		},
		showSnackbar() {
			this.snackbar = true;
		},
		recropActionImage(action) {
		...
		replaceImageWithFile(target) {
			this.$electron.remote.dialog.showOpenDialog({
				...
				async filename => {
					if (filename) {
						target.image = nativeImage.createFromPath(filename).toDataURL();
					}
				});
		},
