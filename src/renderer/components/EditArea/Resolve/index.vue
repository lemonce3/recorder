...
export default {
	...
	methods: {
		...
		updateAction(action) {
			if (this.saveTimeout[action.id]) {
				clearTimeout(this.saveTimeout[action.id]);
			}

			this.saveTimeout[action.id] = setTimeout(() => {
